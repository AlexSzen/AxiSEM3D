...
void KernerIO::initialize(int dumpTimeKernels, int numKernels) {
	...
	if (XMPI::root()) {
		...
		dims_notime.push_back( 2 * numKernels ); //for real and imag part 
...
void KernerIO::dumpToFile() {
	
	
}

void KernerIO::loadNus(std::vector<int> &Nus, int temp_startElem, int temp_countElem) {
	...
	countElem.push_back(temp_countElem);
	Nus.assign(temp_countElem, 0);
...
void KernerIO::loadWavefield(vec_vec_ar6_RMatPP &disp, std::vector<int> &Nus, int &totSteps, int temp_startElem, int temp_countElem) {
	...
	if (XMPI::root()) {
		...
		mNetCDF_r->getVarDimensions("displacement_wavefield", dims); //just to get time length
		
		mNetCDF_r->close();
		totSteps = dims[0];
		XMPI::bcast(&totSteps,1,0);
	}
	mNetCDF_r->openParallel(fname_wvf);
	...
	// fill disp with 0 
	vec_ar6_RMatPP initBuf(totNuProc, zero_ar6_RMatPP);
	disp.assign(totSteps, initBuf);
	
	for (int it = 0; it < totSteps; it++) {//have to read one by one 
		mNetCDF_r->readVariableChunk("displacement_wavefield", disp[it], startElemNu, countElemNu);
		startElemNu[0]++;
	}
...
void KernerIO::loadMaterial(vec_ar12_RMatPP &materials, std::vector<int> &Nus, int temp_startElem, int temp_countElem) {
